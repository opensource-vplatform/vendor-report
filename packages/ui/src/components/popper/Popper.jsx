...
export default function (props) {
  ...
  const [data, setData] = useState({
    ...
    contentWrapClassName: 'popper-content-wrap-' + uuid(),
  });
  useEffect(() => {
    if (isBoolean(open)) {
      setData({
    ...
    if (data.contentVisible && contentRef.current && ref.current) {
      ...
      if (rect.top + rect.height > document.body.clientHeight) {
        ...
        if (hostRect.top - rect.height >= 0) {
          //上方有位置，展示在上方
          contentRef.current.style.top = `${hostRect.top - rect.height}px`;
        } else {
          //上方位置不够,直接顶着最上边展示
      ...
      if (rect.left + rect.width > document.body.clientWidth) {
        ...
        if (hostRect.left - rect.width >= 0) {
          //左边有位置，展示在左方
          contentRef.current.style.left = `${hostRect.left - rect.width}px`;
        } else {
          //左边位置不够,直接顶着最左边展示
