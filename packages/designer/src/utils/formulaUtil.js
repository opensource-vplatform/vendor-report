  ...
  if ((1 === colCount && 1 === rowCount) || isSpan) {
    ...
    if (rang) {
      if (
        ...
        formulaArgs = rangeToFormula(rang, row, col, allRelative, r1c1Style);
      }
    }
    const formulaText = '=' + formula + '(' + formulaArgs + ')';
    ...
    if (sheet.getText(rowEnd, col)) {
      if (rowEnd + 1 < sheet.getRowCount()) {
        const formulaText =
          ...
          formula +
          '(' +
          rangeToFormula(selection, rowEnd + 1, col, allRelative, r1c1Style) +
          ')';
    ...
    if (sheet.getText(row, colEnd)) {
      if (colEnd + 1 < sheet.getColumnCount()) {
        const formulaText =
          ...
          formula +
          '(' +
          rangeToFormula(selection, row, colEnd + 1, allRelative, r1c1Style) +
          ')';
    ...
    if (getSelectionType(selections) === SelectionTypes.OnlyCells) {
      ...
      for (let i = 0, len = selections.length; i < len; i++) {
        ...
        if (!(row + rowCount > sheetRowCount)) {
          const table = sheet.tables.find(firstSelectRow, firstSelectCol);
          if (table) {
          ...
          try {
            setFormulaBySelection(selection, formula, sheet, spread);
          } catch (e) {}
        }
      }
    }
