...
import { Button, CheckBox, OperationDialog } from '@toone/report-ui';
import { getCellTagPlugin, uuid } from '@toone/report-util';
import {
  getRuleTitle,
  RuleFormat,
  ...
  return (
    <HLayout data-type='row' data-selected={isCurrent} onClick={onClick}>
      {columns.map((column, index) => {
        return (
          <RowWrap data-type='cell' key={index} style={{ width: column.width }}>
...
export default function (props) {
  ...
  const handleDel = () => {
    ...
    let newRules = [...rules];
    const removedItem = newRules.splice(currentIndex, 1);
    ...
    dispatch(
      setRuleManagerConfig({
        ...
        rules: newRules,
        removed: [...ruleManagerConfig.removed, ...removedItem],
      })
    );
  };
  const handleMoveUp = () => {
    ...
    if (spread && ruleManagerConfig.rules.length == 0) {
      ...
      for (let row = 0; row < rowCount; row++) {
        for (let col = 0; col < colCount; col++) {
          const plugin = getCellTagPlugin(sheet, row, col, 'rowColumnVisible');
          if (plugin) {
            rules.push(rowColumnVisiblePluginToJson(plugin, row, col));
          }
        }
      }
      dispatch(
  ...
  const handleApply = () => {
    ...
    exeCommand(spread, Commands.ConditionStyle.Reset, {
      configs,
      removed: ruleManagerConfig.removed,
    });
  };
  return (
    <Fragment>
          ...
          columns={[
            ...
            },
            {
              ...
              render: (value, row, datas, column) => {
                ...
                const formula =
                  '=' + GC.Spread.Sheets.CalcEngine.rangesToFormula(ranges);
                return (
                  <Range
                    hostId={data.domId}
                    style={{
                    ...
                    value={formula}
                    onStartSelect={() => dispatch(setRuleManagerVisible(false))}
                    onEndSelect={() => dispatch(setRuleManagerVisible(true))}
                    onChange={(val) => {
                      let ranges = null;
                      try {
                        const result =
