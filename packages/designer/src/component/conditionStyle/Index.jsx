...
import { isArray, isFunction } from '@toone/report-util';
...
import {
  ...
  getIconSetMenu,
  getRowColumnVisibleRulesMenu,
  getTopBottomRulesMenu,
  toConditionMenuType,
  toNormalMenu,
} from './Utils';
import RowColumnVisibleCompareDialog from './RowColumnVisibleCompareDialog';
import RowColumnVisibleBetweenDialog from './RowColumnVisibleBetweenDialog';
...
const Condition_Menu_Datas = [
  ...
  toConditionMenuType(
    'rowColumnVisibleRules',
    '行列显示规则',
    HighlightCellsRulesIcon,
    [
      ...withHandler(getRowColumnVisibleRulesMenu()),
      'divider',
      withHandler({
        value: 'topBottomRulesMoreRules',
        title: '其他规则...',
...
export default function (props) {
  ...
  const applyRowColumnVisibleCompareSetting = (rule) => {
    applyRule(rule);
    closeRowColumnVisibleCompareDialog();
  };

  const applyRowColumnVisibleBetweenSetting = (rule) => {
    applyRule(rule);
    closeRowColumnVisibleBetweenDialog();
  };
  return (
    <Fragment>
      ...
      {conditionStyle.rowColumnVisibleCompareVisible ? (
        <RowColumnVisibleCompareDialog
          onCancel={closeRowColumnVisibleCompareDialog}
          onConfirm={applyRowColumnVisibleCompareSetting}
        ></RowColumnVisibleCompareDialog>
      ) : null}
      {conditionStyle.rowColumnVisibleBetweenVisible ? (
        <RowColumnVisibleBetweenDialog
          onCancel={closeRowColumnVisibleBetweenDialog}
          onConfirm={applyRowColumnVisibleBetweenSetting}
        ></RowColumnVisibleBetweenDialog>
      ) : null}
    </Fragment>
  );
